...
// const AgenticSearchBar = ({ onSearch }) => {
...
//           placeholder="Ask the Agent: 'Best historic places in Mumbai'..."
...
//           className="bg-slate-900 text-white px-4 py-3 rounded-xl font-bold text-sm hover:bg-blue-600 transition-all flex items-center gap-2"
//         >
//           <Search size={18} />
//           Get Plans
//         </button>
...
const AgenticSearchBar = ({ onSearch, onClear, isAiResults }) => {
  const [query, setQuery] = useState("");
    ...
    <div className="relative group">
      ...
      <div className="relative bg-white border border-slate-200 rounded-2xl p-2 flex items-center shadow-xl">
        ...
        <input
          ...
          onChange={(e) => setQuery(e.target.value)}
          onKeyDown={handleKeyDown}
          placeholder="Ask the Agent: 'Historic Mumbai places'..."
          className="flex-grow bg-transparent border-none outline-none px-4 text-slate-700 font-medium placeholder:text-slate-400"
        />
        ...
        {!isAiResults ? (
          <button
            onClick={handleSearch}
            className="bg-slate-900 text-white px-6 py-3 rounded-xl font-bold text-sm hover:bg-blue-600 transition-all flex items-center gap-2"
          >
            <Search size={18} />
            Get Plans
          </button>
          ...
          <button
            onClick={handleClear}
            className="bg-red-500 text-white px-6 py-3 rounded-xl font-bold text-sm hover:bg-red-600 transition-all flex items-center gap-2"
          >
            <X size={18} />
            Clear
          </button>
