...
import { Sparkles, Star, ArrowLeft, Send, MapPin, Clock, ExternalLink, ThumbsUp, MessageCircle, Plus, Navigation, Train, Plane } from 'lucide-react';
import { useNavigate, useParams } from 'react-router-dom';
import { getItineraryById, travelOptions, relatedTrips } from '../data/discoveryItineraries';
import ItineraryRouteMap from '../components/discovery/ItineraryRouteMap';
import TravelRecommendations from '../components/discovery/TravelRecommendations';
import RelatedTrips from '../components/discovery/RelatedTrips';
...
const ItineraryDetails = () => {
  ...
  const [selectedPlaceId, setSelectedPlaceId] = useState(itinerary?.days?.[0]?.places?.[0]?.id || '');
  const [currentLocation, setCurrentLocation] = useState(null);

  const [comments, setComments] = useState(() => {
    const stored = localStorage.getItem(`comments_${itineraryId}`);
    return stored ? JSON.parse(stored) : [
      { id: '1', user: 'Priya', text: 'This itinerary looks amazing! Has anyone tried the morning slot?', likes: 3, liked: false, time: '2 hours ago' },
      { id: '2', user: 'Arjun', text: 'We did this last month. Day 2 was the highlight. Highly recommend!', likes: 7, liked: false, time: '1 day ago' },
      { id: '3', user: 'Sneha', text: 'Can we add a food stop between the first two places? The walk is long.', likes: 2, liked: false, time: '3 days ago' },
    ];
  });
  ...
  const averageReview = useMemo(() => {
    if (!itinerary?.reviews?.length) {
      return 0;
    }
    const total = itinerary.reviews.reduce((sum, review) => sum + review.rating, 0);
    return (total / itinerary.reviews.length).toFixed(1);
  }, [itinerary]);

  const dayData = useMemo(
    () => itinerary?.days?.find((day) => day.day === activeDay) || null,
  ...
  const selectedPlace = useMemo(() => {
    ...
    return dayData.places.find((place) => place.id === selectedPlaceId) || dayData.places[0] || null;
  }, [dayData, selectedPlaceId]);

  const askAgent = async () => {
    if (!question.trim() || !itinerary || !selectedPlace) {
      return;
    }
    setIsLoadingReply(true);
    const reply = await tripService.askAgent(
      `Itinerary: ${itinerary.name}. Day ${activeDay}. Place: ${selectedPlace.name}. Question: ${question}`,
    );
    setAgentReply(reply.message);
  ...
  const addComment = () => {
    ...
    const updated = [
      { id: String(Date.now()), user: commentUser.trim(), text: newComment.trim(), likes: 0, liked: false, time: 'Just now' },
      ...comments,
    ];
    setComments(updated);
    localStorage.setItem(`comments_${itineraryId}`, JSON.stringify(updated));
    setNewComment('');
  };
  ...
  const handleAddToTrip = async () => {
    ...
    const trip = await createTrip({
      ...
      endDate: new Date(Date.now() + (itinerary.days.length - 1) * 86400000).toISOString().split('T')[0],
      notes: itinerary.summary,
      recommendedItineraryId: itinerary.id,
      recommendedItineraryName: itinerary.name,
      itinerary: itinerary.stops.map((stop) => ({
        day: stop.day, time: stop.time, title: stop.name, type: 'Recommended', cost: '$$',
      })),
      destinations: itinerary.stops.map((stop) => ({
        id: stop.id, name: stop.name, day: stop.day, lat: stop.lat, lng: stop.lng,
      })),
      budget: { total: 35000, spent: 0, estimated: 25000 },
    });
  ...
  if (!itinerary) {
    return (
      <div className="max-w-3xl mx-auto py-20 text-center">
        <h2 className="text-3xl font-black text-slate-900">Itinerary not found</h2>
        <button
          onClick={() => navigate('/discovery')}
          className="mt-6 px-6 py-3 rounded-xl bg-slate-900 text-white font-bold"
        >
          Back to Discovery
        </button>
      </div>
    ...
    <div className="max-w-7xl mx-auto px-4 py-8 space-y-8">
      <div className="flex items-center justify-between gap-4 flex-wrap">
        ...
        <div className="flex items-center gap-3 text-slate-600 font-semibold">
          ...
          <Clock size={16} />
          {itinerary.days.length} Days • {itinerary.stops.length} places
        </div>
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-3 bg-slate-50 border-t border-slate-100">
          <div className="flex items-center gap-6 text-sm">
            <span className="flex items-center gap-1.5 text-yellow-600 font-bold"><Star size={14} fill="currentColor" /> {itinerary.rating}</span>
            <span className="flex items-center gap-1.5 text-slate-500"><Clock size={14} /> {itinerary.days.length} Days</span>
            <span className="flex items-center gap-1.5 text-slate-500"><MapPin size={14} /> {itinerary.stops.length} Stops</span>
          </div>
          <span className="text-lg font-black text-blue-600">{itinerary.price}</span>
        </div>
      </motion.section>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <section className="lg:col-span-2 space-y-8">
          <div className="bg-white rounded-3xl border border-slate-100 p-6 shadow-sm">
            <h2 className="text-2xl font-black text-slate-900 mb-4">Day Planner</h2>
                ...
                >
                  <div className="flex items-center gap-2 mb-2">
                    <span className={`w-7 h-7 rounded-lg flex items-center justify-center text-xs font-bold ${
                      selectedPlace?.id === place.id ? 'bg-blue-600 text-white' : 'bg-slate-200 text-slate-600'
                    }`}>{index + 1}</span>
